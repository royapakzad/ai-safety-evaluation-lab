...
const ReasoningLab: React.FC<ReasoningLabProps> = ({ currentUser }) => {
  ...
  const [listFilterLanguage, setListFilterLanguage] = useState<string>('All');
  ...
  const listEvaluationsToDisplay = useMemo(() => {
    const sourceEvals = visibleEvaluations.slice().reverse(); // Show latest first
    if (listFilterLanguage === 'All') {
        return sourceEvals;
    }
    return sourceEvals.filter(ev => {
        const langPair = ev.languagePair || '';
        if (listFilterLanguage === 'Kurdish') {
            return langPair.includes('Kurdish (Kurmanji)') || langPair.includes('Kurdish (Sorani)');
        }
        return langPair.includes(listFilterLanguage);
    });
  }, [visibleEvaluations, listFilterLanguage]);

  const downloadCSV = () => {
    const dataToExport = viewMode === 'list' ? listEvaluationsToDisplay : visibleEvaluations;
             ...
             <div className="flex items-center flex-wrap justify-start sm:justify-end gap-4 mt-4 sm:mt-0">
                 ...
                 {viewMode === 'list' && (
                    <div className="flex items-center gap-1 bg-muted p-1 rounded-lg">
                      {['All', 'Dari/Persian (Farsi)', 'Arabic', 'Pashto', 'Kurdish'].map(lang => (
                          <button
                              key={lang}
                              onClick={() => setListFilterLanguage(lang)}
                              className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${listFilterLanguage === lang ? 'bg-background shadow-sm text-primary' : 'text-muted-foreground hover:bg-background/50'}`}
                          >
                              {lang === 'Dari/Persian (Farsi)' ? 'Farsi' : lang}
                          </button>
                      ))}
                    </div>
                 )}
            ...
            <div className="text-center py-10 bg-card border border-border rounded-xl shadow-sm"><p className="text-lg text-muted-foreground">No comparison evaluations saved yet.</p></div>
             ...
             viewMode === 'list' ? (
                <div className="space-y-8">
                  {listEvaluationsToDisplay.length > 0 ? (
                    listEvaluationsToDisplay.map((ev) => {
                    ...
                    <div className="text-center py-10 bg-card border border-border rounded-xl shadow-sm">
                        <p className="text-lg text-muted-foreground">No evaluations found for the selected language.</p>
                    </div>
                  )}
                </div>
            ) : (
