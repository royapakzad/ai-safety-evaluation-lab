...
const ReasoningLab: React.FC<ReasoningLabProps> = ({ currentUser }) => {
  ...
  const visibleEvaluations = useMemo(() => {
      const baseEvaluations = currentUser.role === 'admin' 
          ? allEvaluations 
          : allEvaluations.filter(ev => ev.userEmail === currentUser.email);
      ...
      return baseEvaluations.filter(ev => 
          (ev.promptA?.toLowerCase().includes(lowercasedQuery)) ||
          (ev.scenarioContext?.toLowerCase().includes(lowercasedQuery)) ||
          (ev.promptB?.toLowerCase().includes(lowercasedQuery))
      );
  ...
  const downloadCSV = () => {
    ...
    if (dataToExport.length === 0) {
        alert("No data to export for the current view/filter.");
        return;
    }
             ...
             <div className="flex items-center flex-wrap justify-start sm:justify-end gap-4 mt-4 sm:mt-0">
                 <div className="relative">
                     <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 text-muted-foreground"><path fillRule="evenodd" d="M9 3.5a5.5 5.5 0 100 11 5.5 5.5 0 000-11zM2 9a7 7 0 1112.452 4.391l3.328 3.329a.75.75 0 11-1.06 1.06l-3.329-3.328A7 7 0 012 9z" clipRule="evenodd" /></svg>
                    </div>
                    <input 
                        type="text"
                        placeholder="Search by scenario or prompt..."
                        value={searchQuery}
                        onChange={e => setSearchQuery(e.target.value)}
                        className="form-input w-full sm:w-64 p-2 pl-10 pr-8 border rounded-md shadow-sm bg-background border-border focus:outline-none focus:ring-2 focus:ring-ring text-sm"
                    ...
                    {searchQuery && (
                        ...
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 text-muted-foreground hover:text-foreground"><path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" /></svg>
                        </button>
                    )}
                 </div>
          ...
          {visibleEvaluations.length === 0 ? (
            <div className="text-center py-10 bg-card border border-border rounded-xl shadow-sm">
                <p className="text-lg text-muted-foreground">
                    {searchQuery.trim() ? `No evaluations match your search for "${searchQuery}".` : 'No comparison evaluations saved yet.'}
                </p>
            </div>
          ) : (
             viewMode === 'list' ? (
